...
+ `IsTagged.cs` - компоненты-теги которые используются просто как bool флаг, рекомендуется называть по аналогии с рекомендацией наименования bool полей, то есть с префиксом Is.
...
+ `SomeAnswer.cs` - компонент-ответ на Request, содержащий результат обработки запроса, найденные данные или причину отказа. По механике Answer похож на Event, но предназначен для системы, породившей Request, либо для другой системы, связанной с породившей Request системой, например внутри одной фичи.
...
+ `SomeSystem.cs` - системы фичи, лежащие в корне папки фичи.
+ `_SomeFeatureModule.cs` - класс реализующий интерфейс `IEcsModule`, добавляющий в пайплайн системы фичи.

Фичи так же удобно группировать через модуль-агрегатор, то есть создавать модуль который в `Import` методе просто добавляет модули других фич. Например в своем проекте те фичи которые могут работать независимо от Unity объединяю в модуль `ProjectCoreModule` и то что работает с Unity объединяю в `ProjectUnityModule`. После такого разделения в `EcsRoot` достаточно просто добавить эти 2 модуля.

### Мета-Атрибуты
Для систем и компонентов, присущих одной фиче, добавлять мета-атрибут MetaGroup, в качестве корневой группы использовать название модуля фичи. Пример:
```c#
// Слово Module из _SomeFeatureModule будет автоматически удалено, останется только _SomeFeature
...
### Модуль-кеш
Удобный способ управления зависимостями систем, это использовать модули как кеш для зависимостей. Модули поддерживают внедрение зависимостей, а так же могут как зависимость внедряться в системы. Таким образом можно все нужные миры, сервисы и прочие зависимости внедрять в модули, а модули внедрять в системы. Теперь код модуля будет выглядеть так:
```c#
class SomeModule: IEcsModule, IInjectionUnit, IEcsInject<EcsDefaultWorld>, IEcsInject<ISomeService1>
{
    void IInjectionUnit.OnInitInjectionBranch(InjectionBranchIniter initer) { initer.AddNode(this); }
    void IEcsInject<EcsDefaultWorld>.Inject(EcsDefaultWorld obj) => _module = obj;
    void IEcsInject<ISomeService1>.Inject(ISomeService1 obj) => _module = obj;
